...
## 2026-07-10 v6 P2: トピッククラスタ配線監査+穴埋め（MediaXAI「p2進めよう」）✅
- factoring手法移植: 14カテゴリ×ピラーのup/down機械分析。判明した穴=**職種11%/11%(P1で深掘りした勝ち筋が未配線!)**・地域0%/0%・言語18%/18%・career42%/14%等
- 一括配線: up=8クラスタ42記事の関連記事グリッド先頭にピラーカード挿入(関連記事</h2>アンカーregex・全記事適合)/down=6ピラー(career-path+15職種一覧/salary-by-language+9/regional-remote+5/remote-jobs+7/women-friendly-companies+6/how-to-choose+5)にカテゴリ記事一覧節を新設
- 再監査結果: 全14クラスタup77-100%/down40-100%(職種・言語・地域・働き方・比較・career=100%/100%)
- sitemap lastmod47件・deploy両push・本番(ピラーdown節3+スポークup3サンプル)検証・GSC再送信
- 残: v6 P3(資格×年代)/P4(登録リクエスト確認→再計測)

## 2026-07-10 v6 P3: 資格×年代（MediaXAI「p3いこう」）✅
- 実測: 「python 資格 50代」29位/6i=cert-python配信・学び直し系23-35位=reskilling-leave配信を確認
- cert-pythonに「40代・50代からのPython資格の活かし方」追記: 40代=QA/DX推進/RPA転身での本気度証明(age-40s既存記述と整合)・**50代=未経験転職は難易度高と正直明記し現職効率化/副業/学び直しの価値へ再フレーム**+教育訓練給付(年齢上限なし)/マナビDX/reskilling-leave/cert-itpassport接続+FAQ2(50代転職の現実/年齢制限なしの支援制度)
- reskilling-leave関連記事にcert-pythonカード(相互接続)。dateModified/sitemap 2件・deploy両push・本番検証・GSC再送信
- ★v6完遂(P1職種×女性15本/P2クラスタ配線42up+47down/P3資格×年代)。残: P4=登録リクエスト実施確認→再計測(MediaXAI待ち)・効果観測2-4週

## 2026-07-11 v7策定（MediaXAI「ネクストアクション策定して欲しい」）→P0リスト提出済み・P1/P2承認待ち
- 全128URL検査: **インデックス率47%(61/128)が最大ボトルネック**。「検出-未登録」47件に収益核ほぼ全部(diagnosis/techgo2件/主要レビュー6社/公的支援クラスタ7本/職種多数/negotiation)=v5/v6施策の大半が未クロール。crawled2/unknown18
...
## 2026-07-12 v7 P2: beginner深掘り（MediaXAI「P2 すすめて」）✅
- 実測: 「未経験 女性 エンジニア」37.6位i10・「未経験からエンジニア 女性」39.0位i4等5クエリ全てbeginner配信。ページは633行(6/12 Tier Sリライト済)だが**公的支援への本文接続ゼロ・診断導線ゼロ**がギャップ
- 新セクションid=public-support「女性向けの公的支援・給付金で費用を抑える」を独学/スクール比較(sec5)直後に追加: 「2択の前の第3の選択肢」枠で東京都/埼玉/教育訓練給付/マナビDX 4カード+全国マップ+ひとり親inline+診断(8つの質問)導線。公的支援クラスタ8本と本文結線
- FAQ2問追加(faqData.mainEntity=schema/inline共通リテラル): 無料公的支援の有無/未経験女性の不利有無(女性比率18.8%=既存出典値のみ・断定なし)。目次11項目再構成(新id挿入で既存アンカー不変更)・関連記事+全国マップ・最終更新/dateModified 7/12
- sitemap lastmod・deploy両push・本番検証(支援6/6・FAQ2・診断・更新日)cache回避until・GSC再送信・報告(id 1525701022734090293)
- ★v7残: P0登録リクエスト20URL(MediaXAI・催促済み)/P3=実施3-4日後の全数再計測

## 2026-07-12 v7 P3: 全数再計測（MediaXAI「p3進めて」）✅
- 全128URL検査: 62/128=48%(7/11比+1でほぼ横ばい)だが**unknown 18→6に激減**(diagnosis/levtech/type-woman/workport/negotiation等が「検出済み」へ昇格・frontendはcrawledまで前進)=P1トップ動線で発見の問題は解消。discovered 47→57・crawled 2→3。残unknown6=reworks/type-vs-doda/workport-vs-recruit/technical-interview/data/category-index
- クエリ(7/5-11): **育成事業クラスタ4.5〜5.1位定着・7日で7クリック**(「女性itエンジニア育成事業」5.1位i27c3/口コミ4.5位/東京都口コミ3.1位)・デジタルカレッジ7.5位登場・DB女性7.5位・未経験36.7位(P2反映待ち)
- ★律速確定: 優先20URLが全て「検出済み・クロール待ち」のまま=**P0登録リクエスト未実施**。報告(id 1525760873413480520)で再依頼。実施後3-4日で再計測を約束
- ★v7完遂(P1トップ動線/P2 beginner/P3再計測)。残: P0(MediaXAI)→再計測/育成事業3位内・未経験の観測(来週)

## 2026-07-12 v8策定（MediaXAI「悩む項目・意思決定に必要な情報を考え抜いて実行計画に」）→承認待ち
- 悩み体系5領域を時系列で定義: A=タイミング(転職×妊娠出産の制度要件=育休の入社1年未満労使協定除外・給付金被保険者期間。scheduleに断片のみ) B=見抜く(厚労省女性活躍推進DB・えるぼし/くるみん・面接での聞き方文例。women-friendly-companiesにDB手順なし) C=お金(出産手当金/育児休業給付金の計算基礎・時短年収影響=専用なし) D=キャリア持続(マミートラック=既存3本で部分カバー) E=最終決定(offer-meetingに女性版受諾前チェックリストなし)
- v8提案(id 1525869895131070606): P1=新規3本(①タイミング設計②見極め実務=DB使い方+聞き方文例③育休・時短のお金)全て一次到達確認+制度変更注記 P2=修正4本(women-friendly-companiesにDB手順/offer-meetingにチェックリスト/reverse-questions・schedule接続) P3=クラスタ配線 P4=登録リクエスト+観測
- 設計根拠: B/Cは公式DB・公式計算式の案内=公的支援クラスタ(4.5-5.1位でクリック獲得中)と同じ一次情報勝ちパターンの横展開

## 2026-07-13 v8 P1: 意思決定サポートクラスタ3本新設（MediaXAI「p1進めよう」）✅
- ホストが一次URL7本を事前到達確認(positivedbはcurl403/ブラウザUA200・協会けんぽ出産手当金と出生後休業支援の正URLをWebSearchで特定)→並列agent3体で新設:
  - career-lifeevent-timing: 育休の労使協定1年未満除外/給付金12か月要件+**前職通算(離職1年以内)**/ケース別4例/産休は勤続要件なし。agentが指示URLの1本=両立支援等助成金ページ(事業主向け)と見抜き特設サイト+Q&A+bosei-naviへ出典差し替え(誠実対応)
  - company-check-guide: 女性活躍推進DB4.3万社の使い方+公表項目を定義ページから転記/えるぼし5項目段階基準を公式PDF実読(2026年4月えるぼしプラス反映)/くるみん/有報+EDINET/聞き方文例6(×→○形式)+タイミング3区分
  - maternity-money-guide: 出産手当金2/3式/育児休業給付67→50%非課税+免除で手取り8割相当/**2025年4月新制度2つ**(出生後休業支援13%=80%手取り10割相当・育児時短就業10%)/年金の養育期間みなし措置/月30万円当てはめ例(目安明記)。上限額は8月改定のため書かず公式誘導
...
## 2026-07-30 ③beginner強化（MediaXAI「次のアクション進めてください」）✅
- 実測: beginnerはtitle/結論box/FAQ/公的支援節/被リンク41本と最適化済み→サジェスト実需で真のギャップ特定=**年代系(40代/30代/30歳)がサブ意図最上位なのに年代本文節なし・age記事リンク0本**。やめとけ/きつい系はサジェスト無し=作らない
- [修正]beginner: 「30代・40代から未経験で目指す場合」節新設(目次11・FAQ12へ・30代=前職掛け算+段階ルート/40代=教育訓練給付・デジタルカレッジ軸・**東京都事業39歳以下の年齢要件注意ボックス**=公式確認済み事実のみ)+age-30s/age-40s接続+結論boxに公的無料育成1行(SERP実査で東京都公式が上位=Googleが評価する角度)
- [修正]ranking: 「女性 エンジニア 転職」29.9位の着地がranking(query+page実測)→meta desc冒頭を「女性エンジニアの転職に強い〜9社」直球化(titleは5.2位クエリ保護で不変更)
- dateModified/最終更新→07-30・sitemap lastmod2件・tsc0/build・deploy両push・本番全項目検証・GSC再送信。報告id 1532246832711995503
- ★7/29戦略の①②③(taro実行分)完了。残=登録リクエスト9URL実施(MediaXAI)→3-4日後再計測/8社アフィリ支給/効果観測2-3週

## 2026-08-10 P1: 9月クリフ対策（MediaXAI「p1進めよう」）✅
- 公式8/10再確認(変化なし: 後期2期生9/1募集中=令和8年度ラスト)。tokyo-women-trainingの募集状況ボックス直下に「募集終了後の選択肢」box新設(デジタルカレッジ/リスキリング事業/教育訓練給付/全国マップ=9月以降の需要をサイト内受け皿へ)
- training-not-selectedに「令和9年度の募集はいつ？【定点更新】」節新設(8/10時点で公式発表なしと正直明記・確認先2つ・推測は書かない方針明文化・目次追加)
- 月次ウォッチ運用開始: 毎月上旬に公式確認→令和9年度発表が出たら当日中に両ページ更新+チャンネル報告(一番槍の鮮度が勝ち筋)。※cronはセッション限定のためメモリ定例として管理
- dateModified/最終更新→08-10・sitemap lastmod2件・deploy両push・本番全項目検証・GSC再送信。報告id 1536247854018854964
